...
async function mailSender(email, otp) {
    ...
    let mailOptions = {
        ...
        to: email,
        subject: "StayEase.in: Your One-Time Password (OTP)",
        text: `
        Dear User,

        Thank you for using StayEase.in To ensure the security of your account, we have sent you a one- time password (OTP) that is valid for the next 5 minutes.
        ...
        Best regards, The StayEase.in Team
...
async function resetPasswordMailSender(email, token) {
    ...
    let mailOptions = {
        from: process.env.EMAIL,
        to: email,
        subject: "Reset Password email from StayEase",
